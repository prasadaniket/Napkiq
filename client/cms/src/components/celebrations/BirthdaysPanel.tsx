...
import { Search, Gift, MessageSquare, Calendar, ChevronRight, Sparkles, Clock, CheckCircle, AlertTriangle } from 'lucide-react'
...
function Initials({ name }: { name: string }) {
  ...
  return (
    <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-red-50 text-xs font-black uppercase text-[#D64238] border border-red-100 shadow-sm transition-transform duration-300 group-hover:scale-105">
      {ini.toUpperCase()}
    </div>
  )
}
...
export default function BirthdaysPage() {
  ...
  const fetchBirthdays = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get<BirthdayCustomer[]>(`/cms/customers/birthdays?${q.toString()}`)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-3xl border border-slate-100 bg-white/80 backdrop-blur-md p-6 shadow-[0_8px_30px_rgb(0,0,0,0.015)] group">
        <div className="absolute -right-10 -top-10 h-32 w-32 rounded-full bg-red-500/5 blur-2xl transition-opacity duration-300 group-hover:opacity-100 pointer-events-none" />
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <div className="flex items-center gap-2.5">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-red-50 text-[#D64238]">
                <Gift className="h-5.5 w-5.5 animate-bounce" />
              </div>
              <h1 className="text-2xl font-black tracking-tight text-slate-900">Birthdays This Month</h1>
            </div>
            <p className="text-xs font-medium text-slate-400 mt-1">
              {outletId ? 'Filtered by outlet · ' : ''}
              {!loading ? `${filteredCustomers.length} customer${filteredCustomers.length !== 1 ? 's' : ''} celebrating` : 'Loading birthdays…'}
            </p>
          </div>
      ...
      {!loading && !error && (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-white/80 backdrop-blur-sm border border-slate-100 p-4 rounded-2xl flex flex-col justify-between hover:shadow-sm transition-all">
            ...
            <p className="text-[10px] font-medium text-slate-400 mt-1">Customers with birthday in current month</p>
          </div>
          ...
          <div className="bg-[#D64238]/5 border border-[#D64238]/10 p-4 rounded-2xl flex flex-col justify-between hover:bg-[#D64238]/10 transition-colors">
            <span className="text-[10px] font-bold uppercase tracking-wider text-[#D64238] flex items-center gap-1">
              <Sparkles className="h-3.5 w-3.5 fill-[#D64238]/10" /> Marketing Insight
            </span>
            <p className="text-[10px] font-semibold text-[#D64238]/80 mt-2 leading-relaxed">
              Birthday campaigns offer a <strong>24% higher convert rate</strong> on average when sent 5 days in advance.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white/70 backdrop-blur-sm border border-slate-100 p-4 rounded-2xl">
          ...
          <div className="relative w-full sm:w-80">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 text-xs border border-slate-100 rounded-xl bg-slate-50/50 focus:bg-white focus:border-[#D64238]/40 outline-none transition-all placeholder:text-slate-400 font-medium"
            />
          </div>
      ...
      <div className="bg-white/80 backdrop-blur-md border border-slate-100 rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.015)]">
        ...
        {error && (
          <div className="flex flex-col items-center justify-center py-12 text-center">
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-red-50 text-[#D64238] mb-3">
              <AlertTriangle className="h-6 w-6" />
            </div>
            <h4 className="text-sm font-extrabold text-slate-800">Failed to Load Birthdays</h4>
        ...
        {!loading && !error && filteredCustomers.length === 0 && (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <h4 className="text-sm font-extrabold text-slate-800">No Birthdays Found</h4>
            <span className="text-xs font-semibold text-slate-400 mt-1">
              {searchTerm || statusFilter !== 'all' 
                ? 'No celebrating customers match your active search filters.'
                : 'No birthdays registered in the system for this month.'
              }
            </span>
          </div>
        )}
              ...
              >
                ...
                <div className="flex items-center gap-3.5 md:w-[30%] min-w-0">
                  ...
                  <div className="min-w-0">
                    <Link href={`/customers/${c.id}`} className="block truncate text-sm font-black text-[#D64238] hover:text-[#B82E25] hover:underline no-underline">
                      {c.fullName}
                    </Link>
                ...
                <div className="flex items-center gap-2 md:w-[20%]">
                  <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-red-50 text-[#D64238] border border-red-100/60">
                  ...
                  <div>
                    ...
                    <span className="block text-xs font-black text-slate-700 mt-0.5">
                      {format(new Date(c.birthDate), 'dd MMMM')}
                    </span>
                  </div>
                </div>
