...
import type { Order, OrderStatus, OrderItem, OrderItemStatus, OrderEvent, Outlet, MenuCategory, MenuItem } from '@/types/api'
...
// Client-side mirror of the server's derivation, so an item toggle moves the card to
// the right column instantly (the SSE echo reconciles the authoritative value).
function deriveOrderStatus(items: { itemStatus: OrderItemStatus }[]): OrderStatus {
...
// Lines are grouped within a card by how far along they are: still cooking (pending)
// on top, plated (ready) next, gone-out (served) at the bottom. So marking a line
// Ready visibly moves it down into the ready group; Served pushes it further ahead.
const ITEM_RANK: Record<OrderItemStatus, number> = { pending: 0, ready: 1, served: 2 }
...
function OrderCard({ order, accent, onAdvance, onCancel, onItemStatus, busy, busyItems }: {
  order: Order
  accent: string
  onAdvance: () => void
  onCancel: () => void
  onItemStatus: (item: OrderItem, next: OrderItemStatus) => void
  busy: boolean
  ...
  // Per-line toggles only make sense once the order is being made (not while it's a
  // brand-new inbox card, and not after it's served/cancelled).
  const showItemToggles = order.status === 'preparing' || order.status === 'ready'

  // While the order is being made, order the lines by progress (pending → ready →
  // served) so a line visibly moves when you advance it. Before that, keep kitchen
  // order. Stable: same-status lines keep their original sequence.
  const rows = showItemToggles
    ? order.items
        .map((it, i) => ({ it, i }))
        .sort((a, b) => ITEM_RANK[a.it.itemStatus] - ITEM_RANK[b.it.itemStatus] || a.i - b.i)
        .map(x => x.it)
    : order.items
  ...
  return (
    <div className="order-card" style={{ borderLeft: `3px solid ${accent}` }}>
      <div className="order-card-header">
        <div className="order-card-board">
          <span className="order-card-num">#{order.dailyNumber ?? '—'}</span>
          <span className="order-card-service">
            {order.serviceType === 'table' ? (
              <><Coffee size={12} />Table {order.boardNumber || '—'}</>
            ) : (
      ...
      <ul className="order-card-items-list">
        {rows.map(it => {
          ...
          const itemBusy = busyItems.has(it.id)
          return (
            <li key={it.id} className={`order-card-item-row status-${st}`}>
              ...
              {showItemToggles && (
                <div className="order-item-toggles">
                  <button
                    type="button"
                    className={`item-toggle-btn ready ${st === 'ready' || st === 'served' ? 'on' : ''}`}
                    disabled={itemBusy || st === 'served'}
                    onClick={() => onItemStatus(it, st === 'ready' ? 'pending' : 'ready')}
                    title={st === 'pending' ? 'Mark ready' : 'Ready'}
                  >
                  ...
                  <button
                    type="button"
                    className={`item-toggle-btn served ${st === 'served' ? 'on' : ''}`}
                    disabled={itemBusy || st === 'pending'}
                    onClick={() => onItemStatus(it, st === 'served' ? 'ready' : 'served')}
                    title={st === 'pending' ? 'Mark ready first' : st === 'served' ? 'Served' : 'Mark served'}
                  >
      ...
      <div className="order-card-footer">
        <span className={`order-card-source-pill ${order.source}`}>
          {order.source === 'staff' ? (<><Users size={9} />Staff</>) : (<><User size={9} />Customer</>)}
        </span>
        <div style={{ display: 'flex', gap: 6 }}>
          <button className="btn-card-cancel" disabled={busy} onClick={onCancel}>Cancel</button>
          {next && (
            <button className="btn-card-advance" style={{ background: accent }} disabled={busy} onClick={onAdvance}>
...
export default function KdsPage() {
  ...
  const updateItemStatus = async (order: Order, item: OrderItem, next: OrderItemStatus) => {
    setBusyItems(prev => new Set(prev).add(item.id))
    // Optimistic: apply the line, re-derive the card's column, let the SSE echo reconcile.
    const nextItems = order.items.map(it => (it.id === item.id ? { ...it, itemStatus: next } : it))
    mergeOrder({ ...order, items: nextItems, status: deriveOrderStatus(nextItems) })
    try {
      await api.patch(`/cms/orders/${order.id}/items/${item.id}/status`, { itemStatus: next })
    } catch (err: any) {
      toast.error(err.response?.data?.error || 'Failed to update item')
      mergeOrder(order) // revert
    } finally {
      setBusyItems(prev => { const n = new Set(prev); n.delete(item.id); return n })
    }
  }
  ...
  return (
    <div className="kds-page-container">
      {/* Premium dark command-centre header */}
      ...
      <div className="page-content" style={{ padding: 0 }}>
        {loading && orders.length === 0 ? (
          <div className="kds-loading-state">
            <Activity className="animate-spin" size={24} />
          ...
          <div className="kds-columns-board">
            {COLUMNS.map(col => {
              ...
              const Icon = col.icon
              return (
                <div key={col.status} className={`kds-board-column kds-col-${col.classKey}`}>
                    ...
                    {items.length === 0 ? (
                      ...
                      items.map(order => (
                        <OrderCard
                          ...
                          onItemStatus={(item, next) => updateItemStatus(order, item, next)}
                        />
                      ))
                    )}
                  </div>
      ...
      <style>{`
        ...
        .kds-header {
          ...
          background: linear-gradient(135deg, #0d0f1a 0%, #141728 100%);
          border-radius: 16px;
        ...
        .btn-new-order {
          ...
          box-shadow: 0 4px 14px rgba(214, 66, 56, 0.35);
          letter-spacing: 0.01em;
        }
        .btn-new-order:hover:not(:disabled) {
          background: var(--color-primary-hover);
          transform: translateY(-1.5px);
          box-shadow: 0 6px 20px rgba(214, 66, 56, 0.45);
        }
        .btn-new-order:active:not(:disabled) { transform: translateY(0); }
        .btn-new-order:disabled { opacity: 0.45; cursor: not-allowed; }
        ...
        @media (max-width: 992px) {
          .kds-columns-board { grid-template-columns: 1fr; }
        }
        ...
        .kds-board-column:hover { border-color: var(--color-border-strong); }

        /* Column tints */
        .kds-col-new { background: rgba(99, 102, 241, 0.022); }
        .kds-col-preparing { background: rgba(245, 158, 11, 0.022); }
        .kds-col-ready { background: rgba(16, 185, 129, 0.022); }
        ...
        .empty-state-text-wrap { display: flex; flex-direction: column; gap: 5px; }
        .empty-state-title {
          font-size: 14px;
          font-weight: 800;
        ...
        .order-card:hover {
          ...
          box-shadow: 0 8px 24px rgba(0, 2, 29, 0.06);
        ...
        .order-card-age.normal { background: rgba(0,2,29,0.04); color: var(--color-text-2); }
        .order-card-age.warning { background: rgba(245,158,11,0.08); color: #d97706; }
        ...
        .order-card-item-name { flex: 1; font-weight: 600; }
        ...
        .item-toggle-btn:hover:not(:disabled) {
          border-color: var(--color-border-strong);
          color: var(--color-text-1);
        }
        .item-toggle-btn:disabled { opacity: 0.35; cursor: not-allowed; }
        .item-toggle-btn.ready.on {
          background: rgba(16, 185, 129, 0.1);
          color: #059669;
          border-color: rgba(16, 185, 129, 0.25);
        }
        .item-toggle-btn.served.on {
        ...
        .btn-card-advance:active:not(:disabled) { transform: translateY(0); }
