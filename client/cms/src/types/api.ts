...
export interface OrderSummary {
  ...
  // Settled-tab totals by payment method (POS counter billing).
  payments:            Record<PaymentMethod, number>
  paymentCounts:       Record<PaymentMethod, number>
  settledTabs:         number
}

// ─── POS table sessions (counter billing) ───────────────────────────────────────

export type PaymentMethod = 'cash' | 'upi' | 'card'
export type SessionStatus = 'open' | 'billed' | 'settled' | 'cancelled'

/** A table's running tab, priced server-side — returned by /cms/sessions. */
...
export interface FloorTable {
  ...
  // Present only on the POS floor (/cms/sessions/floor): the live running tab, if any.
  session?: {
    id:           string
    status:       SessionStatus
    total:        number
